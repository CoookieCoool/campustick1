...
export const ticketService = {
  book:         (eventId, quantity = 1) => api.post(`/tickets/book/${eventId}`, { quantity }).then((r) => r.data),
  getMyTickets: ()         => api.get("/tickets/my").then((r) => r.data),
  getById:      (id)       => api.get(`/tickets/${id}`).then((r) => r.data),
  scan:         (qrData)   => api.post("/tickets/scan", { qrData }).then((r) => r.data),
};

export const orderService = {
  create:  (eventId, quantity = 1) => api.post(`/orders/create/${eventId}`, { quantity }).then((r) => r.data),
  confirm: (orderId)               => api.post(`/orders/confirm/${orderId}`).then((r) => r.data),
  getById: (orderId)               => api.get(`/orders/${orderId}`).then((r) => r.data),
};
